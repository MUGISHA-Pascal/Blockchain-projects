...
const CONTRACT_ADDRESS = "0x427355Bb4648Dd14f770c66DDD1Ae35a181B4596"; // You'll need to replace this with your actual contract address
...
export default function TicketsDApp() {
  ...
  const transferTicket = async () => {
    ...
    try {
      ...
      const tx = await contract.transferTicket(
        Number.parseInt(transferTicketId),
        transferAddress
      );
      await tx.wait();
      ...
      toast({
        title: "Error",
        description:
          "Failed to transfer ticket. Please check ownership and try again.",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  const verifyTicket = async () => {
    ...
    try {
      ...
      const isValid = await contract.verifyTicket(
        Number.parseInt(verifyTicketId)
      );

      toast({
        title: "Ticket Verification",
        description: `Ticket #${verifyTicketId} is ${
  ...
  const invalidateTicket = async () => {
    ...
    try {
      setLoading(true);
      const tx = await contract.invalidateTicket(
        Number.parseInt(invalidateTicketId)
      );
      await tx.wait();
      ...
      toast({
        title: "Error",
        description:
          "Failed to invalidate ticket. Please check ownership and try again.",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
      ...
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600">
            Decentralized ticket management on the blockchain
          </p>
        </div>
        ...
        {!isConnected ? (
          <Card className="max-w-md mx-auto">
            <CardHeader className="text-center">
              ...
              <CardDescription>
                Connect your MetaMask wallet to start managing tickets
              </CardDescription>
            </CardHeader>
            <CardContent>
                      ...
                      <Card key={ticket.id} className="border-2">
                        <CardHeader className="pb-2">
                          <CardTitle className="text-lg flex items-center justify-between">
                            ...
                            <Badge
                              variant={
                                ticket.isValid ? "default" : "destructive"
                              }
                            >
                        ...
                        <CardContent>
                          ...
                          <p className="text-sm text-gray-500 font-mono mt-1">
                            {ticket.owner.slice(0, 6)}...
                            {ticket.owner.slice(-4)}
                          </p>
                        </CardContent>
                      </Card>
                    ))}
                ...
                <Card>
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <Button
                      ...
                      disabled={
                        loading || !transferTicketId || !transferAddress
                      }
                      className="w-full"
                    >
              ...
              <TabsContent value="invalidate">
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      Mark a ticket as invalid (only owner can do this)
                    </CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-4">
                    <div>
                    ...
                    <Alert>
                      <AlertDescription>
                        Warning: This action cannot be undone. Only the ticket
                        owner can invalidate a ticket.
                      </AlertDescription>
                    </Alert>
                    <Button
